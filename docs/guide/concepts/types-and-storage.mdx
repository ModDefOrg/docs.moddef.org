...
- **`byte_order`** — byte order within a single 16-bit word (`BIG_ENDIAN` /
  `LITTLE_ENDIAN`).
- **`word_order`** — order of the words themselves for values wider than one
  register (`WORD_BIG_ENDIAN` / `WORD_LITTLE_ENDIAN`).
...
- `{ struct_ref: ... }` / `{ array: ... }` — composite logical types.
...
A single register can pack several values — for example a time slot with the
hour in the high byte and the minute in the low byte. **Register fields** name
each bit range so you get a struct instead of a raw number:
