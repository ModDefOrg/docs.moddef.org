...
// Browser-side ModDef Transport over a WebSocket bridge, for devices that only
// speak Modbus TCP (which the browser can't open directly). Adapted from
// moddef-ts/examples/browser-ws-bridge/ws-transport.ts; run that repo's
// `bridge.ts` (`npx tsx bridge.ts --tcp <ip>`) to expose a TCP device as a
// WebSocket the dashboard can reach.
